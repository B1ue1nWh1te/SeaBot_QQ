...
from .data_source import get_weibo, get_weibo_detail, get_zhihu, get_zhihu_detail, get_cctv, get_cctv_detail, get_tonghuashun, get_tonghuashun_detail
...
async def weibo_reminder():
    now = time.strftime("%m{}%d{} %H:%M:%S").format('月', '日')
    bot = get_bot()
...
async def weibo_got(bot: Bot, event: GroupMessageEvent, state: T_State):
    if "详情-" in state["number"] and len(state["number"]) > 3:
        number = state["number"][3:].split(",")
        now = time.strftime("%m{}%d{} %H:%M:%S").format('月', '日')
...
async def zhihu_handle(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if args:
        ...
        now = time.strftime("%m{}%d{} %H:%M:%S").format('月', '日')
        data = await get_zhihu(config.zhihu_amount)
        msg = f"「{nickname}·知乎热榜」[CQ:at,qq={event.get_user_id()}]\n\n[{now}]\n{data}"
        await zhihu.send(Message(msg))
...
async def _():
    for i in range(len(config.weibo_reminder_time_hours)):
        scheduler.add_job(weibo_reminder, 'cron', hour=config.weibo_reminder_time_hours[i], minute=config.weibo_reminder_time_minutes[i], id=f"weibo_reminder_{i}")
    for i in range(len(config.zhihu_reminder_time_hours)):
        scheduler.add_job(zhihu_reminder, 'cron', hour=config.zhihu_reminder_time_hours[i], minute=config.zhihu_reminder_time_minutes[i], id=f"zhihu_reminder_{i}")
    for i in range(len(config.cctv_reminder_time_hours)):
        scheduler.add_job(cctv_reminder, 'cron', hour=config.cctv_reminder_time_hours[i], minute=config.cctv_reminder_time_minutes[i], id=f"cctv_reminder_{i}")
    for i in range(len(config.tonghuashun_reminder_time_hours)):
        scheduler.add_job(tonghuashun_reminder, 'cron', hour=config.tonghuashun_reminder_time_hours[i], minute=config.tonghuashun_reminder_time_minutes[i], id=f"tonghuashun_reminder_{i}")
